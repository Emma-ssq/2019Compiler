...
namespace {
  ...
  private:
    ...
    struct LiveReg {
      ...
      bool Dirty = false;              ///< Register needs spill.  Dirty
...
void RegAllocFast::allocateInstruction(MachineInstr &MI) {
  ...
  // If this is a copy, 是一个赋值指令，只有一个src和一个dst，we may be able to coalesce.
  ...
  for (unsigned i = 0, e = MI.getNumOperands(); i != e; ++i) {
    ...
    if (TargetRegisterInfo::isVirtualRegister(Reg)) {
      ...
      /// 如何理解isUse（)
      if (MO.isUse()) {
        //set the constraint：TIED_TO：0:Must be allocated the same register as.
        //or 以前就设置为hasTieOps
  ...
  for (unsigned I = 0; I != DefOpEnd; ++I) {
    ...
    // reg：虚拟寄存器
    /// defineVirtReg(MI, I, Reg, CopySrcReg)
    /// Allocates a register for VirtReg and mark it as dirty.
    /// 调用alloc函数calcspill函数来计算代价选择寄存器
...
void RegAllocFast::allocateBasicBlock(MachineBasicBlock &MBB) {
  ...
  //保证在一个block初始时的虚拟寄存器到物理寄存器的map映射是空的，因此在每个block分配完毕后都要将map，clear
  assert(LiveVirtRegs.empty() && "Mapping not cleared from last block?");

  MachineBasicBlock::iterator MII = MBB.begin();

  // Add live-in registers as live.
  //遍历这个basicblock中的每个live interval，考虑对应的physreg是否被分配
  ...
  // sequentially allocate each instruction in the MBB.
  for (MachineInstr &MI : MBB) {
    LLVM_DEBUG(
      dbgs() << "\n>> " << MI << "Regs:";
      dumpState()
    );
    //处理debug模式下的一些操作，不影响源码指令
  ...
  //在最终把所有的使用了虚拟寄存器溢出，进行写操作，删除virtPhymap
